...
void mouseReleased() {
  if (!estaArrastando) {
    ...
    if (mouseButton == LEFT) {
      ...
      int novaCor = 0;
      int preto = 0, verde = color(0,255,0), vermelho = color(255,0,0), azul = color(0,0,255), amarelo = color(255,255,0);
      
      if (coresCelulas.containsKey(chave)){
        int corAtual = coresCelulas.get(chave);
        if (corAtual == preto) novaCor = verde;
        if (corAtual == verde) novaCor = vermelho;
        if (corAtual == vermelho) novaCor = azul;
        if (corAtual == azul) novaCor = amarelo;
        if (corAtual == amarelo) coresCelulas.remove(chave);
        else coresCelulas.put(chave, novaCor);
      }else coresCelulas.put(chave, preto);
